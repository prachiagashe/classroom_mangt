    ...
    <div class="flex flex-1 items-center justify-center px-6">
        <div class="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md mx-auto">
            ...
            <p class="text-gray-600 mb-6 text-center text-sm">
                Enter your registered email address to receive an OTP to reset your password.
            </p>
            ...
            @if(session('success'))
                <div class="bg-green-100 text-green-600 p-3 rounded mb-4 text-sm font-semibold border border-green-200">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('password.email') }}">
                ...
                <button type="submit" class="w-full bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 transition duration-200 font-semibold mb-4">
                    Send OTP
                </button>
                
                <div class="text-center">
                    <a href="{{ route('login') }}" class="text-blue-600 hover:underline text-sm font-medium">Back to Login</a>
                </div>
            </form>
        </div>
    </div>
